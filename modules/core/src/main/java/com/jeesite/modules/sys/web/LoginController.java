...
public class LoginController extends BaseController {
	...
	public String login(HttpServletRequest request, HttpServletResponse response, Model model) {
		...
		// 是否显示验证码
		model.addAttribute("isValidCodeLogin",
				ObjectUtils.toInteger(Global.getConfig("sys.login.failedNumAfterValidCode", "200")) == 0);
	...
	public String loginFailure(HttpServletRequest request, HttpServletResponse response, Model model) {
		...
		if (!UnauthorizedException.class.getName().equals(exception)) {
			model.addAttribute("isValidCodeLogin",
					BaseAuthorizingRealm.isValidCodeLogin(username, (String) paramMap.get("deviceType"), "failed"));
		}
